...
type daySixteen struct {
	logger *log.Logger
}

func (d *daySixteen) runE(_ *cobra.Command, _ []string) error {
	conf, err := config.New(2022, 16)
	if err != nil {
...
func (d *daySixteen) partOne(scanner *bufio.Scanner) {
	d.logger.Debug("----------> Part One")
}

func (d *daySixteen) partTwo(scanner *bufio.Scanner) {
